...
void loop() {
  ...
  if (cmd == "stand") {
    setFootPos(0, KEEP, KEEP, STAND_Z_F, 1000);
  ...
  if (cmd == "walk") {
    for (int i=0; i<100; i++) {
      setFootPos(0, walkPosF2[step%8][0], walkPosF2[step%8][1], walkPosF2[step%8][2], 200);
...
void setFootPos(int footNum, double x, double y, double z, double time) {
  ...
  speedMap[footNum][0] = length_x / (time/10);
  speedMap[footNum][1] = length_y / (time/10);
  speedMap[footNum][2] = length_z / (time/10);
